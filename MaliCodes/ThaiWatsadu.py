...
for i in filenames:
  ...
  with pd.ExcelWriter(splitPath+'\\Split-'+fileExcel) as writer:
          df.insert(3,'Productname2',' ')
          df.insert(6,'Pack',1)
          df.Pack = df.apply(ceilingPack, axis=1)
          df.insert(7,'pricePack',df.Pack*df.price)
          df.insert(8,"Specialprice",(df.price))
          df['Specialprice']=df.apply(checkprice, axis = 1)
          df.insert(8,"priceX",(df.Specialprice*2))
          df.insert(10,"Profit",(df.Specialprice-df.price))          
